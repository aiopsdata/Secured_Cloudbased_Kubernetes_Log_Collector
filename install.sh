...
while getopts :nclmeki:u:p:s: flag
do
    case "${flag}" in
     c) cloud="1";;
     i) cloud_id=${OPTARG};;
...
then
	...
	if [[ $log == "0" && $metric == "0" && $elasticsearch == "0" ]]
	then
		...
		then 
			...
			helm install elasticsearch eskibana --values eskibana/values.yaml --namespace=aiops
		fi
