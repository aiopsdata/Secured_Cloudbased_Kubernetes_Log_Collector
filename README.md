...
### Architecture <br/>
**Architecture of Log Collector consist of five mircoservices(PODS):** <br/><br/>
...
***Log Collector:*** The job of Log Collector is collect Pod Logs and Events generated from kubernetes system and store them in Elasticsearch database. Log Collector internally consist of two python scripts:
1. PodLog script: It will collect logs of all the running pods inrespective of their namespace and store them into Elasticsearch.
2. Events script: It will collect all the events happened in Kubernetes system and store them into Elasticsearch.

***Node-Exporter:*** Prometheus gives us verity of kubernetes cluster information and we mannage deployment of prometheus using Node-Exporter.<br/><br/>
***Metriccollector:*** It is designed to collet Metrices and Prometheus data and store it into Elasticsearch. Metriccollector consist of two python scripts.
1. Metric script: It collects metric data generated from kuberentes cluster and store it into Elasticsearch.
...
2. **Local Based Elasticsearch With Security** (`./install.sh `) <br/>
     In this option you are require to give SYSTEM_NAME and YOUR_NAME at installation time. During installation you will get your creds (USER_NAME and PASSWORD) for Elasticsearch and Kibana. In this option all the collected data will be stored under YOUR_NAME index in Elasticsearch.
