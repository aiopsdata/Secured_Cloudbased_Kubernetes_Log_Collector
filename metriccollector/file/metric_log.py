...
while(True):
    ...
    if os.environ["CLOUD"] == "cloud":
        ...
        try: 
            ...
            es = Elasticsearch(es_link, timeout = 80)
    ...
    for namespace_data in namespace_list.items:
        ...
        for pod in data["items"]:
            if len(pod['containers']) == 0:
                continue
            det = datetime.now()
            data_dic = json.dumps(dict({'user':os.environ["ESUSER"],'type':'metric','agent' : "metric-collector",'datetime': det.strftime("%d/%m/%Y %H:%M:%S"),'data' : pod['containers'][0],'time': round(time.time(), 2)}))
            
            try:
                es.index(index=os.environ['INDEX_NAME'],id=document_id,body=data_dic,ignore=400)     # creating document in elasticsearch
            except ElasticsearchException as e:
