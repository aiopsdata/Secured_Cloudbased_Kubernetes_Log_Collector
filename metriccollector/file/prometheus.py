...
def fetch_from_node_exporter_url(url):
    ...
    try:
        response_data = {}
        response_data['user'] = os.environ["USER"]
        response_data['type'] = "prometheus"
...
def insert_record_es(response_data):
    try:
        es_conn.index(index=os.environ['PINDEX_NAME'],body=response_data,ignore=400)  # Insert data in ElasticSearch
    except ElasticsearchException as e:
        print("\n Exception when calling ElasticsearchApi->insert_record: %s\n" % e)    

if os.environ["CLOUD"] == "cloud":
    try:
        es_conn = Elasticsearch([os.environ["CLOUD_ID"]],use_ssl=True,verify_certs=False,ca_certs=False,ssl_show_warn=False,http_auth=(os.environ["USER"],os.environ["PASSWORD"]),timeout=80)
    ...
    try:
        ...
        es_conn = Elasticsearch(es_link, timeout = 80)
